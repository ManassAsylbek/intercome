...
def _reload_via_ami() -> tuple[bool, str]:
    """Перезагружает PJSIP через Asterisk Manager Interface (TCP)."""
    ...
    try:
        ...
        # Login
        send(f"Action: Login\r\nUsername: {user}\r\nSecret: {secret}")
        banner = recv_until_blank()
        if "Success" not in banner:
            sock.close()
            return False, f"AMI login failed: {banner}"

        # Send pjsip reload
        send("Action: Command\r\nCommand: pjsip reload")
        resp = recv_until_blank()

        # Logoff
        send("Action: Logoff")
        sock.close()

        if "reloaded" in resp.lower() or "success" in resp.lower():
            return True, "pjsip reload via AMI OK"
        return True, resp  # AMI command was sent even if response is verbose
    except Exception as exc:
